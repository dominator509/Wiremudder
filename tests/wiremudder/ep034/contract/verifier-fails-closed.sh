#!/usr/bin/env sh
# EP-034 M1 contract test: the node verifier exists, is executable, and fails
...
[ -f scripts/node-verifiers/EP-034.sh ] || fail "missing node verifier"
[ -x scripts/node-verifiers/EP-034.sh ] || fail "node verifier not executable"

# Unknown subcommands must fail closed.
if scripts/node-verifiers/EP-034.sh NO_SUCH_SUBCOMMAND >/dev/null 2>&1; then
  fail "verifier accepted unknown subcommand"
fi

# Nonexistent milestone subcommands must fail closed.
if scripts/node-verifiers/EP-034.sh M99 >/dev/null 2>&1; then
  fail "verifier accepted nonexistent milestone M99"
fi

echo "contract EP-034 verifier-fails-closed: ok"
