#!/usr/bin/env sh
# EP-035 M4 performance test: real measured distribution for the release
# core hot paths, within the SPEC-004 budget (P4, 1 ms).
set -eu
cd "$(dirname "$0")/../../../.."
...
out=$(mktemp /tmp/ep035_perf_XXXX.log)
...
grep -q "^perf release:" "$out" || fail "perf sentinel missing"

python3 - "$out" <<'PY' || fail "perf distribution check failed"
import re, sys
out = open(sys.argv[1]).read()
m = re.search(r"perf release: p50_us=(\d+) p95_us=(\d+) max_us=(\d+) budget_us=(\d+) samples=(\d+)", out)
...
print(f"perf release: p50={p50}us p95={p95}us max={mx}us budget={budget}us samples={samples} OK")
PY

echo "performance EP-035 release-core: ok"
