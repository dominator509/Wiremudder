#!/usr/bin/env sh
# EP-032 M5 requirement test: WM-SPEC-027-R06 - "Performance tests record hardware, OS, build, workload, repetitions, distributions, thresholds, and raw artifacts."
...
grep -q "WM-SPEC-027-R06" .agent/specs/*.md || fail "WM-SPEC-027-R06 missing from specs"

# 2. Validation matrix maps it to EP-032, LF-032, and this test path.
grep -q "WM-SPEC-027-R06.*EP-032.*LF-032" .agent/requirements/VALIDATION_MATRIX.tsv \
  || fail "matrix row for WM-SPEC-027-R06 missing EP-032/LF-032"
grep -q "tests/wiremudder/ep032/requirements/wm-spec-027-r06" .agent/requirements/VALIDATION_MATRIX.tsv \
  || fail "matrix row for WM-SPEC-027-R06 missing test path"
...
echo "requirements WM-SPEC-027-R06: ok"
