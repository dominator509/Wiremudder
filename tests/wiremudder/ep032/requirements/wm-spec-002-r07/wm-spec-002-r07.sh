#!/usr/bin/env sh
# EP-032 M5 requirement test: WM-SPEC-002-R07 - "Transport and encoding selection is completed in EP-005 using repository evidence and benchmarks, then pinned by ADR; no executor invents an unverified library API."
...
grep -q "WM-SPEC-002-R07" .agent/specs/*.md || fail "WM-SPEC-002-R07 missing from specs"

# 2. Validation matrix maps it to EP-032, LF-032, and this test path.
grep -q "WM-SPEC-002-R07.*EP-032.*LF-032" .agent/requirements/VALIDATION_MATRIX.tsv \
  || fail "matrix row for WM-SPEC-002-R07 missing EP-032/LF-032"
grep -q "tests/wiremudder/ep032/requirements/wm-spec-002-r07" .agent/requirements/VALIDATION_MATRIX.tsv \
  || fail "matrix row for WM-SPEC-002-R07 missing test path"
...
echo "requirements WM-SPEC-002-R07: ok"
