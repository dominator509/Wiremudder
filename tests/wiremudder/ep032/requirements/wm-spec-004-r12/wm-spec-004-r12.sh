#!/usr/bin/env sh
# EP-032 M5 requirement test: WM-SPEC-004-R12 - "Performance readiness requires distributions, hardware profile, workload, raw evidence, and regression thresholds rather than a single anecdotal timing."
...
grep -q "WM-SPEC-004-R12" .agent/specs/*.md || fail "WM-SPEC-004-R12 missing from specs"

# 2. Validation matrix maps it to EP-032, LF-032, and this test path.
grep -q "WM-SPEC-004-R12.*EP-032.*LF-032" .agent/requirements/VALIDATION_MATRIX.tsv \
  || fail "matrix row for WM-SPEC-004-R12 missing EP-032/LF-032"
grep -q "tests/wiremudder/ep032/requirements/wm-spec-004-r12" .agent/requirements/VALIDATION_MATRIX.tsv \
  || fail "matrix row for WM-SPEC-004-R12 missing test path"
...
echo "requirements WM-SPEC-004-R12: ok"
