#!/usr/bin/env sh
# EP-032 M5 requirement test: WM-SPEC-002-R09 - "A component may be replaced only behind a reversible feature flag after differential compatibility, performance, failure, and rollback proofs pass."
...
grep -q "WM-SPEC-002-R09" .agent/specs/*.md || fail "WM-SPEC-002-R09 missing from specs"

# 2. Validation matrix maps it to EP-032, LF-032, and this test path.
grep -q "WM-SPEC-002-R09.*EP-032.*LF-032" .agent/requirements/VALIDATION_MATRIX.tsv \
  || fail "matrix row for WM-SPEC-002-R09 missing EP-032/LF-032"
grep -q "tests/wiremudder/ep032/requirements/wm-spec-002-r09" .agent/requirements/VALIDATION_MATRIX.tsv \
  || fail "matrix row for WM-SPEC-002-R09 missing test path"
...
echo "requirements WM-SPEC-002-R09: ok"
