...
# Every copilot UI source file must be listed in the build.
for f in src/wiremudder/ui/copilot/*.cpp; do
  [ -e "$f" ] || continue
  base=$(basename "$f")
  grep -q "^    ${base}$" src/CMakeLists.txt \
    || fail "copilot UI source ${f} exists but is not compiled (add ${base} to mudlet_SRCS in src/CMakeLists.txt)"
done
