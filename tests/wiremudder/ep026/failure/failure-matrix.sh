#!/usr/bin/env sh
# EP-026 M4 failure test: the real controlled failure matrix must run
# through the production wire-soundscape crate and prove every required
...
out=$(mktemp /tmp/ep026_failure_XXXX.log)
...
grep -q "failure matrix EP-026: ok" "$out" || fail "failure matrix not ok"

echo "failure EP-026 matrix: ok"
