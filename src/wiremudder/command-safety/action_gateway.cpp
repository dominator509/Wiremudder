...
QString actionSourceName(ActionSource s) {
    switch (s) {
        ...
        case ActionSource::Autopilot: return "autopilot";
        case ActionSource::Voice: return "voice";
        case ActionSource::Macro: return "macro";
        case ActionSource::Trigger: return "trigger";
        case ActionSource::Script: return "script";
        case ActionSource::Plugin: return "plugin";
        case ActionSource::Headless: return "headless";
        case ActionSource::CrossSession: return "cross-session";
    }
    return "unknown";
}

QVector<ActionSource> allActionSources() {
    return {ActionSource::Ai,          ActionSource::Autopilot, ActionSource::Voice,
            ActionSource::Macro,       ActionSource::Trigger,   ActionSource::Script,
            ActionSource::Plugin,      ActionSource::Headless,  ActionSource::CrossSession};
}

QString riskTierName(RiskTier t) {
    switch (t) {
        case RiskTier::Safe: return "safe";
        case RiskTier::Standard: return "standard";
...
QString denialReasonName(DenialReason r) {
    switch (r) {
        case DenialReason::EmergencyStop: return "emergency-stop";
        case DenialReason::NotConnected: return "not-connected";
        case DenialReason::RoutingUnstable: return "routing-unstable";
        case DenialReason::InjectionFlagged: return "injection-flagged";
        case DenialReason::DeniedByPolicy: return "denied-by-policy";
        case DenialReason::AutomationDisabled: return "automation-disabled";
        case DenialReason::Pacing: return "pacing";
        case DenialReason::QueueFull: return "queue-full";
    }
    return "unknown";
}
...
static bool looksDestructive(const QString& command, const QStringList& args) {
    ...
    static const QStringList keys = {"kill",   "quit",  "quit!",    "delete", "drop all",
                                     "sacrifice", "sell all", "give all"};
...
CommandPolicy CommandDatabaseQt::evaluate(const QString& command, const QStringList& args) const {
    for (const CommandRule& rule : m_rules) {
        ...
        if (rule.deny) {
            return CommandPolicy{command, rule.tier, true, false, false,
                                 "denied by command database"};
        }
        QString reason;
    ...
    return CommandPolicy{command,
                         destructive ? RiskTier::Risky : RiskTier::Standard,
                         false,
                         destructive,
                         true,
                         "unknown command; standard tier"};
}
...
bool HumanTempoQt::shouldSend(quint64 nowMs, quint64* waitMs) {
    if (nowMs - m_burstStartMs >= m_burstWindowMs) {
        ...
        if (m_lastSendMs > 0 && nowMs - m_lastSendMs < m_minIntervalMs) {
            if (waitMs) *waitMs = m_minIntervalMs - (nowMs - m_lastSendMs);
            return false;
        }
    }
    if (m_burstCount >= m_maxBurst) {
        if (waitMs) *waitMs = m_burstWindowMs - (nowMs - m_burstStartMs);
        return false;
    }
    m_lastSendMs = nowMs;
    m_burstCount += 1;
...
QJsonObject ActionProposal::toJson() const {
    ...
    QJsonArray argsArr;
    for (const QString& a : args) argsArr.append(a);
...
bool VisibleQueueQt::remove(const QString& proposalId) {
    ...
    for (int i = m_entries.size() - 1; i >= 0; --i) {
        if (m_entries[i].proposalId == proposalId) m_entries.removeAt(i);
    }
    return m_entries.size() != before;
}

ActionGatewayQt::ActionGatewayQt(const CommandDatabaseQt& db, const HumanTempoQt& tempo,
                                 int queueCapacity)
    : m_db(db), m_tempo(tempo), m_queue(queueCapacity) {}

bool ActionGatewayQt::propose(ActionSource source, const QString& suggestion,
                              ActionProposal* out, QString* err) {
    const QString trimmed = suggestion.trimmed();
    if (trimmed.isEmpty()) {
        if (err) *err = "empty suggestion";
        return false;
    }
    if (trimmed.size() > 1024) {
        if (err) *err = "oversized suggestion";
        return false;
    }
    if (!m_db.isReady()) {
        if (err) *err = "command database unavailable";
        return false;
    }
...
GateDecision ActionGatewayQt::evaluate(const ActionProposal& proposal, const GateContext& ctx) const {
    if (m_emergencyStop.blocks()) return GateDecision::Denied;
    if (!ctx.connected) return GateDecision::Denied;
    if (!ctx.profileAutomationEnabled) return GateDecision::Denied;
    if (ctx.injectionFlagged) return GateDecision::Denied;
    if (!ctx.routingStable) return GateDecision::Denied;
    const CommandPolicy policy = m_db.evaluate(proposal.normalizedCommand, proposal.args);
    if (policy.denied || !policy.argOk) return GateDecision::Denied;
...
ActionAuditEntry ActionGatewayQt::makeAudit(const ActionProposal& p, const QString& result,
...
bool ActionGatewayQt::approveAndSend(const ActionProposal& proposal, const GateContext& ctx,
    ...
    if (d == GateDecision::NeedsConfirmation) {
        ...
        if (result) *result = "needs-confirmation";
        if (err) *err = QString();
        return true;
    }
    quint64 waitMs = 0;
