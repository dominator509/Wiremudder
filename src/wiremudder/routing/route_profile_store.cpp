...
bool routeKindEnabled(RouteKind k) {
    return !(k == RouteKind::InterfaceBinding || k == RouteKind::VmNetns ||
             k == RouteKind::SelfHostedRelay);
}

const char* routeKindLabel(RouteKind k) {
    switch (k) {
        case RouteKind::Direct: return "direct";
        case RouteKind::System: return "system";
        case RouteKind::Socks5: return "socks5";
        case RouteKind::Socks4a: return "socks4a";
        case RouteKind::HttpConnect: return "http-connect";
        case RouteKind::TorLocalSocks: return "tor-local-socks";
        case RouteKind::SshDynamicForward: return "ssh-dynamic-forward";
        case RouteKind::VpnMetadata: return "vpn-metadata";
        case RouteKind::InterfaceBinding: return "interface-binding (future)";
        case RouteKind::VmNetns: return "vm-netns (future)";
        case RouteKind::SelfHostedRelay: return "self-hosted-relay (future)";
    }
    return "unknown";
}

RouteProfile RouteProfile::create(const QString& id, const QString& name, RouteKind kind,
    ...
    if (!routeKindEnabled(kind)) {
        if (err) *err = QString("unsupported kind: %1").arg(routeKindLabel(kind));
        return r;
    }
    r.id = id;
    r.name = name;
...
bool RouteProfile::fromJson(const QJsonObject& obj, RouteProfile* out, QString* err) {
    ...
    if (kind == "direct") r.kind = RouteKind::Direct;
    else if (kind == "system") r.kind = RouteKind::System;
    else if (kind == "socks5") r.kind = RouteKind::Socks5;
    else if (kind == "socks4a") r.kind = RouteKind::Socks4a;
    else if (kind == "http-connect") r.kind = RouteKind::HttpConnect;
    else if (kind == "tor-local-socks") r.kind = RouteKind::TorLocalSocks;
    else if (kind == "ssh-dynamic-forward") r.kind = RouteKind::SshDynamicForward;
    else if (kind == "vpn-metadata") r.kind = RouteKind::VpnMetadata;
    else {
        if (err) *err = QString("unknown kind: %1").arg(kind);
        return false;
    }
    ...
    if (r.id.isEmpty() || r.name.isEmpty()) {
        if (err) *err = "malformed route";
        return false;
    }
    if (obj.value("schema_version").toInt() != ROUTING_SCHEMA_VERSION) {
        if (err) *err = "schema version mismatch";
        return false;
    }
    *out = r;
    if (err) *err = QString();
...
bool RoutingStoreQt::addRoute(const RouteProfile& route, QString* err) {
    if (!route.validate(err)) return false;
    if (find(route.id)) {
...
QVector<const RouteProfile*> RoutingStoreQt::list() const {
    ...
    for (const auto& r : m_routes) out.append(&r);
    std::sort(out.begin(), out.end(),
              [](const RouteProfile* a, const RouteProfile* b) { return a->name < b->name; });
    return out;
}
...
bool RoutingStoreQt::remove(const QString& id, QString* err) {
    for (int i = 0; i < m_routes.size(); ++i) {
        if (m_routes[i].id == id) {
            ...
            if (m_selected == id) {
                m_selected.clear();
                appendAudit(id, "selection_cleared",
                            "removed selected route; no silent replacement");
            }
            if (err) *err = QString();
            return true;
        }
...
bool RoutingStoreQt::verifyEgress(const QString& host, int port, EgressResult* out, QString* err) {
    ...
    if (connected) {
        ...
        res.ok = true;
        res.detail = "verified";
        appendAudit(r->id, "egress_verified",
                    "user-triggered egress verification passed");
    } else {
        res.ok = false;
        res.detail = QString("failed: %1").arg(socket.errorString());
        appendAudit(r->id, "egress_failed",
                    QString("user-triggered egress verification failed: %1")
...
bool RoutingStoreQt::loadFromDir(const QString& dir, QString* err) {
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        if (err) *err = "cannot read routing.json";
        return false;
    }
    QJsonParseError pe;
    ...
    if (pe.error != QJsonParseError::NoError) {
        if (err) *err = "malformed routing.json";
        return false;
    }
