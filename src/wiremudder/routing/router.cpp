...
bool RouterQt::connectViaDecision(QTcpSocket* socket, const RouteDecision& d,
    ...
    if (d.routeId.isEmpty()) {
        if (err) *err = "no route selected; blocking connect";
        return false;
    }
    if (!routeKindEnabled(d.kind)) {
        if (err) *err = QString("route kind disabled: %1").arg(routeKindLabel(d.kind));
        return false;
    }
    ...
    if (!socket->waitForConnected(timeoutMs)) {
        if (err) {
            *err = QString("route %1 connect failed: %2")
                       .arg(QString::fromUtf8(routeKindLabel(d.kind)), socket->errorString());
        }
        return false;
    }
    if (err) *err = QString();
