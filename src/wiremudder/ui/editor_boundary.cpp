...
SpellcheckCore::SpellcheckCore() {
    ...
    const char* words[] = {"look", "north", "south", "east", "west", "open",
                           "close", "say", "tell", "shout", "get", "drop",
                           "inventory", "score", "help", "quit", "attack",
                           "cast", "buy", "sell", "wear", "remove"};
    for (const char* w : words) dict_.insert(QString::fromLatin1(w));
}
...
QStringList SpellcheckCore::suggest(const QString& word, int maxDistance,
    ...
    for (const QString& w : dict_) {
        ...
        if (dist <= maxDistance) scored.append({dist, w});
    }
...
QString SpellcheckCore::autocorrect(const QString& word, int maxDistance) const {
    ...
    for (const auto& c : corrections_) {
        if (c.first == lower) return c.second;
    }
...
QStringList CompletionCore::candidates(const QString& prefix, int maxCandidates) const {
    ...
    for (const QString& item : items_) {
        if (item.startsWith(prefix, Qt::CaseInsensitive)) out << item;
