...
QStringList StatusGaugeQt::ids() const {
    QStringList out;
    for (const models::StatusGauge& g : gauges_) out << g.id;
    return out;
}
...
ThemeQt::ThemeQt(QString name) : name_(std::move(name)) {
    const char* def[16] = {"#000000", "#cd0000", "#00cd00", "#cdcd00",
                           "#0000ee", "#cd00cd", "#00cdcd", "#e5e5e5",
                           "#7f7f7f", "#ff0000", "#00ff00", "#ffff00",
                           "#5c5cff", "#ff00ff", "#00ffff", "#ffffff"};
    for (int i = 0; i < 16; ++i) ansi_[i] = QString::fromLatin1(def[i]);
}
...
QStringList ThemeQt::ansiColors() const {
    QStringList out;
    for (int i = 0; i < 16; ++i) out << ansi_[i];
    return out;
}

WorkspaceLayoutQt::WorkspaceLayoutQt(QString name) : name_(std::move(name)) {}
...
bool WorkspaceLayoutQt::fromJson(const QJsonObject& obj) {
    ...
    theme_.setColors(theme.value("fg").toString(QStringLiteral("#d0d0d0")),
                     theme.value("bg").toString(QStringLiteral("#1e1e1e")));
