...
QString optionName(quint8 option)
{
    ...
    case opt::SGA:
        return QStringLiteral("SGA");
    case opt::NAWS:
        return QStringLiteral("NAWS");
    case opt::TERMINAL:
        return QStringLiteral("TERMINAL-TYPE");
    case opt::CHARSET:
        return QStringLiteral("CHARSET");
    case opt::MSSP:
        return QStringLiteral("MSSP");
    case opt::MXP:
        return QStringLiteral("MXP");
    case opt::MSP:
        return QStringLiteral("MSP");
    case opt::GMCP:
        return QStringLiteral("GMCP");
    case opt::MSDP:
        return QStringLiteral("MSDP");
    case opt::ATCP:
        return QStringLiteral("ATCP");
    default:
        return QStringLiteral("OPT-%1").arg(option);
    }
}
...
QList<NegotiationEvent> parseIac(const QByteArray& data, int maxSb)
{
    ...
    int i = 0;
    while (i + 2 < len) {
        if (static_cast<quint8>(p[i]) != cmd::IAC) {
            ++i;
            continue;
        }
        ...
        if (verb == cmd::SB) {
            ...
            bool closed = false;
            while (j + 1 < len) {
                if (static_cast<quint8>(p[j]) == cmd::IAC && static_cast<quint8>(p[j + 1]) == cmd::SE) {
                    out.append({cmd::SB, sbOpt, sub, i});
                    i = j + 2;
                    closed = true;
                    break;
                }
                if (static_cast<quint8>(p[j]) == cmd::IAC && static_cast<quint8>(p[j + 1]) == cmd::IAC) {
        ...
        if (verb == cmd::DO || verb == cmd::DONT || verb == cmd::WILL || verb == cmd::WONT) {
            if (i + 2 < len) {
...
QList<Capability> detectCapabilities(const QList<NegotiationEvent>& events)
{
    ...
    for (quint8 option : options) {
        ...
        const bool negotiated = (lastVerb == cmd::WILL || lastVerb == cmd::DO);
        const bool declined = (lastVerb == cmd::WONT || lastVerb == cmd::DONT);
        Capability c;
    ...
    for (const QString& p : {QStringLiteral("MCP"), QStringLiteral("PUEBLO"), QStringLiteral("SIMUTRONICS-GSL")}) {
        caps.append({p, false, QStringLiteral("research"), QStringLiteral("no inherited option byte; research decision documented")});
    }
    return caps;
}
