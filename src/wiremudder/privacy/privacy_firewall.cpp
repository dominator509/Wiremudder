...
{
    const bool remoteBlocked = (m_mode == PrivacyMode::Disabled || m_mode == PrivacyMode::LocalOnly);
    if (remoteBlocked || m_lockdown) {
        ...
        if (m_deniedCategories.contains(category)) {
            ...
            for (const OverrideEntry& o : m_overrides) {
                if (o.category == category && o.userVisible && o.consentReceiptId.size() >= 16) {
                    overridden = true;
                    break;
                }
            }
...
QString PrivacyFirewall::redact(const QString& text) const
{
    ...
    QString out = text;
    out.replace(QRegularExpression(QStringLiteral("(?i)sk-[a-z0-9]{16,}")), QStringLiteral("[REDACTED:provider-token]"));
    out.replace(QRegularExpression(QStringLiteral("(?i)-----BEGIN [A-Z ]*PRIVATE KEY-----")), QStringLiteral("[REDACTED:signing-metadata]"));
    return out;
}
...
bool PrivacyFirewall::addOverride(const OverrideEntry& entry)
{
    if (!entry.userVisible)
        return false;
    if (entry.consentReceiptId.size() < 16)
        return false;
    if (entry.overrideId.size() < 8)
        return false;
    m_overrides.insert(entry.overrideId, entry);
    return true;
}

bool PrivacyFirewall::grantConsent(const QString& receiptId, const QString& feature, const QString& provider, const QString& dataClass, const QString& profile)
{
    if (receiptId.size() < 16)
        return false;
    m_consent.insert(receiptId, QStringList{feature, provider, dataClass, profile, QStringLiteral("granted")});
    return true;
}
...
bool PrivacyFirewall::isConsented(const QString& receiptId, const QString& feature, const QString& provider, const QString& dataClass, const QString& profile) const
{
    const auto it = m_consent.constFind(receiptId);
    if (it == m_consent.constEnd())
        return false;
    const QStringList& v = it.value();
    return v.value(4) == QStringLiteral("granted") && v.value(0) == feature && v.value(1) == provider && v.value(2) == dataClass && v.value(3) == profile;
}
