...
Read the named active ExecPlan. Confirm the scheduler and lease identify the same node. Execute its milestones in order, use source evidence before inherited edits, run every declared check, close the node only through `scripts/node-verify.sh`, expected-file audit, scope audit, ledger, and green tag.
