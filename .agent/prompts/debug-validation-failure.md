...
Apply the milestone verify-fix ladder to the supplied failing wrapper. Capture full output and exit code, normalize the first meaningful error signature, inspect prior attempts, state one hypothesis, select the correct rung, make one permitted change within the path fence, run the narrowest diagnostic, and append evidence. Do not broaden scope, repeat a diff, invent an upstream symbol, or weaken a gate.
